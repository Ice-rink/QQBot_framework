...
/**
 * 注册后台指令（重构后更名为 addCmd）
 * 原名称 registerBackendCommand 已移除
 */
export function addCmd(name, description, handler) {
...
export function initBackendCommands() {
    ...
    addCmd("help", "显示所有后台指令", async (args) => {
        const log = new Logger({ title: "Main" });
        const lines = ["📋 可用的后台指令:", ""];
    ...
    addCmd("status", "显示系统状态", async (args) => {
        const log = new Logger({ title: "Main" });
        const lines = [
            "📊 系统状态",
            "",
    ...
    addCmd("reload", "重新加载配置文件", async (args) => {
        const log = new Logger({ title: "Main" });
        try {
    ...
    addCmd("clear", "清空终端", async (args) => {
        console.clear();
        return null;
    });

    addCmd("exit", "退出程序", async (args) => {
        const log = new Logger({ title: "Main" });
        log.info("👋 正在退出...");
