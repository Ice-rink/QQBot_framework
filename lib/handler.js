...
export function registerCommand(name, description, handler) {
    SLASH_COMMANDS.set(name.toLowerCase(), { description, handler });
...
export class MessageHandler {
    ...
    async processMessage(msg) {
        ...
        try {
            ...
            // 日志输出
            const sender = msg.senderId || msg.openid || msg.member_openid || 'unknown';
            logger.info(`[${msg.type}] ${sender}: ${msg.content?.slice(0, 50)}`);
...
export function registerBuiltinCommands() {
    ...
    registerCommand("status", "查看状态", async (msg) => {
        const lines = [
            ...
            `队列长度: ${this?.messageQueue?.length || 0}`,
        ];
        return lines.join("\n");
    });

    // /test - 时间戳 (插件已注册，这里作为备用)
    registerCommand("time", "当前时间", async () => {
